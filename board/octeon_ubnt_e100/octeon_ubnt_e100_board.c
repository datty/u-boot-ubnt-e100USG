...
static int write_ubnt_gd_entries(octeon_eeprom_header_t **hdr)
{
    ...
    if ((r = flash_sect_protect(0, gdptr, gdptr + CFG_UBNT_GD_SIZE - 1))) {
        flash_perror(r);
        ret = 1;
        goto out;
    }
    if ((r = flash_sect_erase(gdptr, gdptr + CFG_UBNT_GD_SIZE - 1))) {
        flash_perror(r);
        ret = 1;
        goto out;
    }
    if ((r = flash_write(buf, gdptr, CFG_UBNT_GD_SIZE))) {
        flash_perror(r);
        ret = 1;
        goto out;
    }
...
static struct board_type_info board_types[] = {
    {"e101", BOARD_E101_MAJOR, {0xee, 0x02, 0x07, 0x77}},
    {"e102", BOARD_E102_MAJOR, {0xee, 0x01, 0x07, 0x77}},
                             ...
                             uint8_t *mpr)
{
    ...
    if ((r = flash_sect_protect(0, eptr, eptr + CFG_UBNT_EEPROM_SIZE - 1))) {
        flash_perror(r);
        return 1;
    }
    if ((r = flash_sect_erase(eptr, eptr + CFG_UBNT_EEPROM_SIZE - 1))) {
        flash_perror(r);
        return 1;
    }
                            ...
                            const char *b_minor, const char *serial)
{
    ...
    bd->board_type = htons(CVMX_BOARD_TYPE_UBNT_E100);
