...
BootpHandler(uchar * pkt, unsigned dest, unsigned src, unsigned len)
{
	...
	if (NetReadLong((ulong*)&bp->bp_vend[0]) == htonl(BOOTP_VENDOR_MAGIC))
		BootpVendorProcess((u8 *)&bp->bp_vend[4], len);

	NetSetTimeout(0, (thand_f *)0);

	debug ("Got good BOOTP\n");
        /* Change autoload default to be not autoloading */
        s = getenv("autoload");
        if (s == NULL || *s == 'n') {
                /*
...
#if (CONFIG_COMMANDS & CFG_CMD_DHCP)
	ext_len = DhcpExtended((uchar *)bp->bp_vend, DHCP_DISCOVER, 0, 0);
#else
...
static void DhcpOptionsProcess (uchar * popt)
{
	...
	while (popt < end && *popt != 0xff) {
		...
		case 59:	/* Ignore Rebinding Time Option */
			break;
                case 224:
                        /* Octeon addition for boot command */
                        size = truncate_sz ("Boot command", sizeof (NetOurBootCommand), oplen);
                        memcpy (&NetOurBootCommand, popt + 2, size);
...
static void DhcpSendRequestPkt(Bootp_t *bp_offer)
{
	...
	extlen = DhcpExtended((uchar *)bp->bp_vend, DHCP_REQUEST, NetDHCPServerIP, OfferedIP);
...
DhcpHandler(uchar * pkt, unsigned dest, unsigned src, unsigned len)
{
	...
	case SELECTING:
		...
		if (strncmp(bp->bp_file,
			...
			if (NetReadLong((ulong*)&bp->bp_vend[0]) == htonl(BOOTP_VENDOR_MAGIC))
				DhcpOptionsProcess((uchar *)&bp->bp_vend[4]);
	...
	case REQUESTING:
		...
		if ( DhcpMessageType((uchar *)bp->bp_vend) == DHCP_ACK ) {
			char *s;
			...
			/* Obey the 'autoload' setting, Default to not autoloading */
                        s = getenv("autoload");
                        if (!s || *s == 'n') {
                                /*
                                 * Just use BOOTP to configure system;
