...
extern int octeon_mgmt_eth_initialize(bd_t*);
extern int octeon_spi4000_initialize(bd_t * bis);
extern int octeon_spi_ixf18201_initialize(bd_t * bis);
...
int eth_initialize(bd_t *bis)
{
	...
	rtl8169_initialize(bis);
        ...
        if (!getenv("disable_networking") && octeon_has_feature(OCTEON_FEATURE_MGMT_PORT))
        ...
        if (!getenv("disable_networking") && !getenv("disable_spi"))
            octeon_spi4000_initialize(bis);
#endif
#if defined(OCTEON_SPI_IXF18201_ENET)
        if (!getenv("disable_networking") && !getenv("disable_spi"))
	...
	if (!eth_devices) {
		...
		puts ("Networking disabled by 'disable_networking' env variable.\n");
		...
		do {
			...
			if (memcmp(env_enetaddr, "\0\0\0\0\0\0", 6)) {
                            /* Allow dev MACs starting with 00:BA:DD to be overridden silently */
				if (memcmp(dev->enetaddr, "\0\xBA\xDD", 3) &&
...
void eth_set_current(void)
{
	...
	if (!eth_current)	/* XXX no current */
        ...
        /* Reset bootp count when changing devices.
        ** This doesn't quite do what we want, but is close enough. */
        BootpTry = 0;
