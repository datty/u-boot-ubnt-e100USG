...
char		NetOurBootCommand[256]={0,};	/* Our boot command      	*/
...
restart:
	...
	case 0:
		...
		case TFTP:
			...
			TftpStart();
                        tftp_start_cycles = octeon_get_cycles();
			break;
	...
	for (;;) {
		...
		case NETLOOP_SUCCESS:
			if (NetBootFileXferSize > 0) {
                                int tftp_msecs = (int)((octeon_get_cycles() - tftp_start_cycles)/(gd->cpu_clock_mhz*1000));
                                int bytes_sec = ((int64_t)NetBootFileXferSize * 1000)/tftp_msecs;
                                
				char buf[10];
				printf("Bytes transferred = %ld (%lx hex), %d Kbytes/sec\n",
					NetBootFileXferSize,
					NetBootFileXferSize,
                                        bytes_sec/1024);
				...
				sprintf(buf, "%lX", (unsigned long)net_load_addr);
				setenv("fileaddr", buf);
			}
			eth_halt();
...
unsigned
NetCksum(uchar * ptr_arg, int len)
{
	ulong	xsum;
        ushort *ptr = (ushort *)ptr_arg;
...
{
  sprintf (addr_string, "%x:%x:%x:%x:%x:%x",
