...
/* Load a file from a FAT filesystem, and allocate a named
** bootmem block for the contents
*/
int do_fat_fsload_alloc (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
    ...
    char *alloc_name;
    long file_size;
    int64_t alloced_addr;

	if (argc < 5) {
		printf ("usage: fatloadalloc <interface> <dev[:part]> <addr> <filename> [alloc_name]\n");
		return (0);
	}
    ...
    /* Use either filename or alloc_name argument to name alloced block */
	if (argc == 6)
		alloc_name = argv[5];
	else
		alloc_name = argv[4];

	file_size = file_fat_get_size (argv[4]);
    if (file_size < 0)
    {
        printf("ERROR getting filesize for file: %s\n", argv[4]);
        return(-1);
    }
    /* Allocate a named block for file data */


    alloced_addr = cvmx_bootmem_phy_named_block_alloc(file_size, offset, 0, 0, alloc_name, 0);
    if (alloced_addr < 0)
    {
        printf("Error allocating named block: name: %s, address: 0x%08x\n", alloc_name, offset);
        return(-1);
    }
    offset = (unsigned long)alloced_addr;

	size = file_fat_read (argv[4], (unsigned char *) offset, 0);

	if(size==-1) {
		printf("\n** Unable to read \"%s\" from %s %d:%d **\n",argv[4],argv[1],dev,part);
	} else {

		printf ("\nAllocated block %s at 0x%08x, %ld bytes read\n", alloc_name, offset, size);

		sprintf(buf, "%lX", size);
		setenv("filesize", buf);
        /* Set env. var. indicating address that load went to */
        sprintf(buf, "0x%lX", offset);
        setenv("fileaddr", buf);
        setenv("filename", argv[4]);
	}

	return size;
}
U_BOOT_CMD(
	fatloadalloc,	6,	0,	do_fat_fsload_alloc,
...
int do_fat_fsload (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	if(size==-1) {
        ...
        setenv("fileaddr", buf);
        setenv("filename", argv[4]);
        octeon_check_mem_load_range();
	}

        /* Return 0 so command can repeat */
        return 0;
	//return size;
}
