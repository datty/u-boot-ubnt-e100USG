...
#ifdef CONFIG_LBA48
        /* LBA48 is not supported, but at least allow it to be configured as long
        ** devices don't use the high addressing */
        if ((blknr & 0xFFFFFFFFULL) != blknr)
        {
            printf("ERROR: LBA48 addressing not supported!\n");
            return 0;
        }
#endif
	device &= 0xff;
