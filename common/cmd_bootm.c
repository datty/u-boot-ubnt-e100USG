...
#ifdef CONFIG_HAS_DATAFLASH
	if (addr_dataflash(addr)){
		read_dataflash(data, len, (char *)load_addr);
		data = load_addr;
	}
#endif
...
#if (CONFIG_COMMANDS & CFG_CMD_GUNZIP)
int do_gunzip(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
    int current_arg = 1;
    const cvmx_bootmem_named_block_desc_t *named;
    void *file_addr;
    int file_size;
    void *dest_addr;
    int dest_size;
    ...
    /* See if the first argument is a named block */
    named = cvmx_bootmem_find_named_block(argv[current_arg]);
    if (named)
    {
        /* Use the size and address of the named block */
        file_addr = (void*)(ulong)named->base_addr;
        file_size = named->size;
        current_arg++;
    }
    else
    {
        /* Not a named block, require the address followed by the size */
        file_addr = (void*)simple_strtoul(argv[current_arg], NULL, 16);
        current_arg++;

        if (argc <= current_arg)
            goto usage;

        file_size = simple_strtoul(argv[current_arg], NULL, 16);
        current_arg++;
    }
    ...
    /* See if the second argument is a named block */
    named = cvmx_bootmem_find_named_block(argv[current_arg]);
    if (named)
    {
        /* Use the size and address of the named block */
        dest_addr = (void*)(ulong)named->base_addr;
        dest_size = named->size;
        current_arg++;
    }
    else
    {
        /* Not a named block, require the address followed by the size */
        dest_addr = (void*)simple_strtoul(argv[current_arg], NULL, 16);
        current_arg++;

        if (argc <= current_arg)
            goto usage;

        dest_size = simple_strtoul(argv[current_arg], NULL, 16);
        current_arg++;
    }

    printf("Uncompressing 0x%x@0x%x to 0x%x@0x%x\n", file_size, (ulong)file_addr, dest_size, (ulong)dest_addr);

    /* We have all of our arguments, do the gunzip */
    int result = gunzip(dest_addr, dest_size, file_addr, &file_size);
    if (result == 0)
    {
        char buf[32];
        sprintf(buf, "%lX", file_size);
        setenv("filesize", buf);
        printf("Uncompressed 0x%x bytes\n", file_size);
        return 0;
    }
    else
    {
...
int gunzip(void *dst, int dstlen, unsigned char *src, int *lenp)
{
	...
	if (r != Z_OK && r != Z_STREAM_END) {
        ...
        if (s.avail_in && !s.avail_out && r != Z_STREAM_END)
        {
            printf ("Error: Destination not large enough\n");
            retval = -1;
            goto out;
        }

        if (!s.avail_in && (r != Z_STREAM_END))
        {
            printf ("Error: Source truncated\n");
            retval = -1;
            goto out;
        }
