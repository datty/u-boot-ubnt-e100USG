...
#include "lib_octeon.h"
    ...
    !defined(CFG_ENV_IS_IN_OCTEON_NAND)	&& \
    !defined(CFG_ENV_IS_IN_RAM)	&& \
    !defined(CFG_ENV_IS_NOWHERE)
# error Define one of CFG_ENV_IS_IN_{NVRAM|EEPROM|FLASH|DATAFLASH|NOWHERE|RAM|OCTEON_NAND}
#endif
...
#if CONFIG_OCTEON
        /* Don't allow the loadaddr env variable to be changed if it is set based on
        ** a reserved block. */
	if (load_reserved_addr && strcmp(argv[1],"loadaddr") == 0)
        {
            printf("loadaddr not set due to load reserved memory allocated at 0x%x.\n", load_reserved_addr);
            printf("Please refer to the bootloader SDK documentation.\n");
            return 1;
        }
#endif
...
#ifdef CONFIG_OCTEON
        /* Have change in console UART take effect immediately */
	if (strncmp(name, "console_uart", 12) == 0) {
            int uart = simple_strtoul(argv[2], NULL, 10);
            if (uart == 0 || uart == 1)
            {
                gd->console_uart = uart;
int octeon_uart_setup(int uart_index);
	...
	*++env = '\0';
	...
	if (strcmp(argv[1], "vga_fg_color") == 0 ||
        ...
        /* If just pci_console_count is set, just set up the data structures for
        ** later use, don't make it the active console. */
	if (strcmp(argv[1], "pci_console_count") == 0)
            uboot_octeon_pci_console_setup();

        /* Treat pci_console_active as a shorthand for changing the std* variables */
        if (strcmp(argv[1], "pci_console_active") == 0)
        {
            /* This rechecks the environment.... */
            uboot_octeon_pci_console_setup();
...
#if defined(CFG_ENV_IS_IN_OCTEON_NAND) || defined(CFG_ENV_IS_IN_NVRAM) || defined(CFG_ENV_IS_IN_EEPROM) || \
    ((CONFIG_COMMANDS & (CFG_CMD_ENV|CFG_CMD_FLASH)) == \
      (CFG_CMD_ENV|CFG_CMD_FLASH))
int do_saveenv (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
#if !CFG_ENV_IS_NOWHERE
    extern char * env_name_spec;
	printf ("Saving Environment to %s...\n", env_name_spec);

	return (saveenv() ? 1 : 0);
#else
    printf("Environment updates not supported\n");
    return(1);
#endif
}
...
#endif	/* CFG_CMD_ENV */
#if defined(CFG_ENV_IS_IN_OCTEON_NAND)
