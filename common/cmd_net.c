...
/* The network loading changes the global load address, which we don't want to do,
** so create a new variable for the networking to use */
uint32_t net_load_addr;  
...
{
    int retval = netboot_common (TFTP, cmdtp, argc, argv);
...
netboot_common (int proto, cmd_tbl_t *cmdtp, int argc, char *argv[])
{
	...
	default: printf ("Usage:\n%s\n", cmdtp->usage);
        ...
        /* Set network global load address */
        net_load_addr = addr;
