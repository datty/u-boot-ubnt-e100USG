...
int  env_init(void)
{
	DECLARE_GLOBAL_DATA_PTR;
        ...
        ** that we can read it.
        ** When the environment is copied to RAM, the flash mappings have been returned to 
        ** normal, so no fixup is required.
        ** Fixup only needed when booting from flash */
        if (!(gd->flags & GD_FLG_RAM_RESIDENT))
        {
            ...
            /* normal_base is what the 'expected' in the non-TLB case, and what the
            ** env_ptr is based on. It will always be >= the currently
...
int saveenv(void)
{
	...
	ulong	flash_offset;
	char	env_buffer[CFG_ENV_SECT_SIZE];
#else
	char *env_buffer = (char *)env_ptr;
#endif	/* CFG_ENV_SECT_SIZE */
	int rcode = 0;

        debug("Saving 0x%x bytes of environment to address 0x%x\n", CFG_ENV_SIZE, CFG_ENV_ADDR);
#if defined(CFG_ENV_SECT_SIZE) && (CFG_ENV_SECT_SIZE > CFG_ENV_SIZE)
