...
void usb_show_tree_graph(struct usb_device *dev,char *pre)
{
	...
	pre[index]=0;
	printf(" %s (%s, %dmA)\n",usb_get_class_desc(dev->config.if_desc[0].bInterfaceClass),
		(dev->speed == USB_DEV_SPEED_LOW) ? "1.5MBit/s" : (dev->speed == USB_DEV_SPEED_FULL) ? "12MBit/s" : "480MBits/s",
...
{
    /* Need to close all cached pipes before we scan the bus */
    usb_close_all_pipes();
    ...
    udelay(500000);  /* Some devices need a delay here in case of repeated scans */
    return 0;
}
/*********************************************************************************
...
U_BOOT_CMD(
	...
	"    to memory address `addr'\n"
        " The 'usb_host_port' environtment variable controls which USB host port is used\n"
        " for devices with more than one port.  Port numbering starts at 0.\n"
);
