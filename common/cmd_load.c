...
int do_load_serial_bin (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	if ((s = getenv("download_baudrate")) != NULL) {
		load_baudrate = simple_strtoul(s, NULL, 0);
	}

	if (argc >= 2) {
		offset = simple_strtoul(argv[1], NULL, 16);
                if (!offset)
                    offset = load_addr;  /* use default load address if offset is 0 */
                {
                }
	}
	if (argc == 3) {
        ...
        /* set  to current baudrate if 0 entered */
		if (load_baudrate == 0)
			load_baudrate = current_baudrate;
	}

	if (load_baudrate != current_baudrate) {
		printf ("## Switching baudrate to %d bps for kermit download...\n",
			load_baudrate);
	...
	if (load_baudrate != current_baudrate) {
		printf ("## Switching to baudrate to %d bps....\n",
			current_baudrate);
		udelay(50000);
		gd->baudrate = current_baudrate;
		serial_setbrg ();
	...
	if (load_ser_addr) {
		...
		if (((s = getenv("autoscript")) != NULL) && (strcmp(s,"yes") == 0)) {
			...
			rcode = autoscript (load_ser_addr);
		}
	}
#endif
