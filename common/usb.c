...
#define MAX_TRIES 5
char *usb_speed_to_string(int speed)
{
...
void usb_hub_port_connect_change(struct usb_device *dev, int port)
{
	...
	USB_HUB_PRINTF("portstatus %x, change %x, %s\n", portstatus, portchange,
		portstatus & (1<<USB_PORT_FEAT_LOWSPEED) ? "Low Speed" : portstatus & (1<<USB_PORT_FEAT_HIGHSPEED) ? "High Speed" : "Full Speed");
	...
	usb=usb_alloc_new_device();
        if (portstatus & (1<<USB_PORT_FEAT_LOWSPEED))
            usb->speed = USB_DEV_SPEED_LOW;
        else if (portstatus & (1<<USB_PORT_FEAT_HIGHSPEED))
            usb->speed = USB_DEV_SPEED_HIGH;
        else
            usb->speed = USB_DEV_SPEED_FULL;

	dev->children[port] = usb;
	usb->parent=dev;


        /* We have found a new device behind a hub, now we need to determine if we need
        ** to use split transactions for it.  This is the case for full/low speed devices behind
        ** high speed hubs.
        ** Look back up through the USB tree to find out which hub/port handle the speed translation */
        {
            int i;
            struct usb_device *usb_ptr;
            if (usb->speed == USB_DEV_SPEED_HIGH)
            {
                /* Don't need to do anything here..... */
                USB_HUB_PRINTF("Found new %s speed device below a hub, not looking for parent speed transition\n", usb_speed_to_string(usb->speed));
            }
            else
            {
                USB_HUB_PRINTF("Found new %s speed device below a hub, looking up to find speed transition\n", usb_speed_to_string(usb->speed));
                usb_ptr = usb;
                while (usb_ptr)
                {
                    if (usb_ptr->parent)
                    {
                        int port = -1;
                        for (i = 0; i < usb_ptr->parent->maxchild; i++)
                        {
                            if (usb_ptr == usb_ptr->parent->children[i])
                                port = i;
                        }
                        USB_HUB_PRINTF("Parent speed is: %s, port is: %d\n", usb_speed_to_string(usb_ptr->parent->speed), port);
                        if (usb_ptr->speed != usb_ptr->parent->speed)
                        {
                            USB_HUB_PRINTF("Speed transition found:  Hub id: %d, port: %d\n", usb_ptr->parent->devnum, port);
                            usb->split_hub_id = usb_ptr->parent->devnum;
                            usb->split_hub_port = port;
                        }
                    }
                    else
                    {
                        USB_HUB_PRINTF("Top of tree reached!\n");
                    }

                    usb_ptr = usb_ptr->parent;
                }
            }
        }
