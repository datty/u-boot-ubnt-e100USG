...
#define virt_to_bus(a)	((uint32_t)(a) & 0x3fffffff)
#define phys_to_bus(a)	((a) & 0x3fffffff)
#define bus_to_phys(a)	((a) | (1 << 31))
#endif
...
#define USE_IO 0
#if USE_IO
#define readl(x)    octeon_pci_io_readl(x)
#define readw(x)    octeon_pci_io_readw(x)
#define readb(x)    octeon_pci_io_readb(x)
#define writeb(val, reg)    octeon_pci_io_writeb(val, reg)
#define writew(val, reg)    octeon_pci_io_writew(val, reg)
#define writel(val, reg)    octeon_pci_io_writel(val, reg)
#else
#define readl(x)    octeon_pci_mem1_readl(x)
#define readw(x)    octeon_pci_mem1_readw(x)
#define readb(x)    octeon_pci_mem1_readb(x)
#define writeb(val, reg)    octeon_pci_mem1_writeb(val, reg)
#define writew(val, reg)    octeon_pci_mem1_writew(val, reg)
#define writel(val, reg)    octeon_pci_mem1_writel(val, reg)
#endif
...
static int rtl8169_init_board(struct eth_device *dev)
{
	...
	for (i = 1000; i > 0; i--)
		if ((RTL_R8(ChipCmd) & CmdReset) == 0)
			...
			udelay(10);
        if (RTL_R8(ChipCmd) & CmdReset)
        {
            printf("ERROR: chip not out of reset!, ChipCmd: 0x%x\n", RTL_R8(ChipCmd));
        }
	...
	printf("PCI device: TxConfig = 0x%hX, tmp: 0x%x\n", (unsigned long) RTL_R32(TxConfig), tmp);
	tpc->chipset = 0;
...
static int rtl_send(struct eth_device *dev, volatile void *packet, int length)
{
	...
	tpc->TxDescArray[entry].buf_addr = cpu_to_le32(virt_to_bus(ptxb));
        printf("Setting buff (bus) addr to: 0x%x, phy addr: 0x%x\n", le32_to_cpu(tpc->TxDescArray[entry].buf_addr), ptxb);
	if (entry != (NUM_TX_DESC - 1)) {
		tpc->TxDescArray[entry].status = cpu_to_le32(
	...
	while ((le32_to_cpu(tpc->TxDescArray[entry].status) & OWNbit) && (currticks() < to));	/* wait */
...
static void rtl8169_init_ring(struct eth_device *dev)
{
	...
	for (i = 0; i < NUM_RX_DESC; i++) {
		if (i == (NUM_RX_DESC - 1))
			tpc->RxDescArray[i].status = cpu_to_le32(
			    (OWNbit | EORbit) + RX_BUF_SIZE);
		else
