...
#define CONFIG_SYS_FLASH_BANKS_LIST { CFG_FLASH_BASE }
#endif
...
__attribute__((unused)) static flash_info_t *flash_get_info(ulong base)
{
	int i;
	flash_info_t * info = 0;
...
static void flash_make_cmd(flash_info_t *info, u32 cmd, void *cmdbuf)
{
	...
	if (use_cfi_hack) {
		for (i = 0; i < info->portwidth; i++) {
			int idx = sizeof(u32) - info->portwidth + i;
			cp[i] = ((uchar *) &cmd)[idx];
		}
	} else {
...
#endif
			cp[cp_offset] = (cword_offset >= sizeof(u32))
			                ? 0x00 : val;
		}
	}
}
...
#else
	retval = use_cfi_hack ? flash_read8(cp)
	                        : flash_read8(cp + info->portwidth - 1);
#endif
			  ...
			  uint offset, uchar cmd)
{
	...
	case FLASH_CFI_16BIT:
		...
		d = flash_read16(addr);
		if (cmd == 'Q' && d != cword.w && le16_to_cpu(d) == cword.w) {
			use_cfi_hack = 1;
			info->chipwidth = 1;
		}
		retval = use_cfi_hack
		         ? (le16_to_cpu(d) == cword.w) : (d == cword.w);
		break;
	case FLASH_CFI_32BIT:
...
#ifndef CONFIG_FLASH_SHOW_PROGRESS
#define CONFIG_FLASH_SHOW_PROGRESS  20
#endif
/*-----------------------------------------------------------------------
...
static void cmdset_amd_read_jedec_ids(flash_info_t *info)
{
	...
	case FLASH_CFI_8BIT:
		info->device_id = flash_read_uchar (info,
						FLASH_OFFSET_DEVICE_ID);
		if (use_cfi_hack && info->device_id == info->manufacturer_id
		    && info->manufacturer_id == 0xc2) {
			uint off = FLASH_OFFSET_DEVICE_ID << 1;
			info->device_id = flash_read_uchar(info, off);
		}
...
static int __flash_detect_cfi (flash_info_t * info, struct cfi_qry *qry)
{
	...
	for (cfi_offset=0;
		...
		if (flash_isequal (info, 0, FLASH_OFFSET_CFI_RESP, 'Q')
			...
			debug ("device interface=%d cfi_off=0x%04X\n",
			       info->interface, info->cfi_offset);
			...
			{
				debug("modify unlock addrs for compat "
				      "chip=%d intf=%d\n", info->chipwidth,
				      info->interface);
			...
			debug("unlock addrs 1=0x%04X 2=0x%04X\n",
