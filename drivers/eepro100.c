...
#elif defined(CONFIG_E500) || defined(CONFIG_DB64360) || defined(CONFIG_DB64460)
...
static int eepro100_init (struct eth_device *dev, bd_t * bis)
{
	...
	for (i = 0;
	     ...
	     i++) {
		if (i >= TOUT_LOOP) {
			printf ("%s: Tx error buffer not ready1, status: 0x%x\n", dev->name, le16_to_cpu (tx_ring[tx_cur].status));
			goto Done;
		}
	}
	...
	for (i = 0; !(le16_to_cpu (tx_ring[tx_cur].status) & CFG_STATUS_C);
		 i++) {
		if (i >= TOUT_LOOP) {
			printf ("%s: Tx error buffer not ready2\n",
				dev->name);
			goto Done;
		}
	}
...
static int eepro100_send (struct eth_device *dev, volatile void *packet, int length)
{
	...
	for (i = 0; !(le16_to_cpu (tx_ring[tx_cur].status) & CFG_STATUS_C);
		 i++) {
		if (i >= TOUT_LOOP) {
			printf ("%s: Tx error buffer not ready3\n", dev->name);
			goto Done;
		}
	}
