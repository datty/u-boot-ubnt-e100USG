...
static void natsemi_reload_eeprom(struct eth_device *dev);
...
static void natsemi_dump_stats(struct eth_device *dev);
...
#undef inl
#undef inb
#define INL(dev, reg) inl(((pci_dev_t)(dev)->iobase) + reg)
#define OUTL(dev, value, reg) outl(value, ((pci_dev_t)(dev)->iobase) + reg)
#define INW(dev, reg) inw(((pci_dev_t)(dev)->iobase) + reg)
#define OUTW(dev, value, reg) outw(value, ((pci_dev_t)(dev)->iobase) + reg)
#define outw(val, addr)     octeon_pci_io_writew(val, addr);inw(addr)
#define inw(addr)      octeon_pci_io_readw(addr)
#define outl(val, addr)     octeon_pci_io_writel(val, addr);inw(addr)
#define inl(addr)      octeon_pci_io_readl(addr)
#define phys_to_bus(a)	octeon_pci_phys_to_bus(a)
#define bus_to_phys(a) octeon_pci_bus_to_phys(a)
#else
...
#ifdef NATSEMI_DEBUG
		printf("natsemi: NatSemi ns8381[56] @ 0x%08x (0x%08x)n", dev->iobase, iobase);
#endif
...
natsemi_init(struct eth_device *dev, bd_t * bis)
{
	...
	OUTL(dev, StatsClear, StatsCtrl); /* Clear Stats */
	return 1;
}
...
static void
natsemi_reload_eeprom(struct eth_device *dev)
{
	int i;

	OUTL(dev, EepromReload, PCIBusCfg);
	for (i=0;i<HW_TIMEOUT;i++) {
		udelay(50);
		if (!(INL(dev, PCIBusCfg) & EepromReload))
			break;
	}
	if (i==HW_TIMEOUT) {
		printf("Warning %s: EEPROM did not reload in %d usec.\n",
			dev->name, i*50);
	}
}
...
static void
natsemi_dump_stats(struct eth_device *dev)
{
#if 0
  printf("%s: TX descriptor register contains: %#08X\n", __FUNCTION__,
	 INL(dev, TxRingPtr));
  printf("%s: RX descriptor register contains: %#08X\n", __FUNCTION__,
	 INL(dev, RxRingPtr));

  printf("Pkts received w/errors: %9d\n", INL(dev, RxPktErrs));
  printf("Missed received packets:%9d\n", INL(dev, RxMissed));
  printf("Receive FCS errors:     %9d\n", INL(dev, RxCRCErrs));
#endif
}
#endif
