			       ...
			       int allowbbt)
{
	...
	if (!(chip->options & NAND_BBT_SCANNED) && chip->scan_bbt) {
			    ...
			    struct mtd_oob_ops *ops)
{
	...
	while(1) {
		...
		if (realpage != chip->pagebuf || oob) {
			...
			else if (!aligned && NAND_SUBPAGE_READ(chip) && !oob && chip->ecc.read_subpage)
			    ...
			    struct mtd_oob_ops *ops)
{
	...
	MTDDEBUG (MTD_DEBUG_LEVEL3, "nand_read_oob: from = 0x%08qx, len = %i\n",
						  ...
						  int busw, int *maf_id)
{
	...
	*maf_id = chip->read_byte(mtd);
	...
	MTDDEBUG (MTD_DEBUG_LEVEL1, "NAND device: Manufacturer ID:"
	...
	if (!type)
        {
            MTDDEBUG (MTD_DEBUG_LEVEL0, "ERROR: NAND device not recognized!\n");
            return ERR_PTR(-ENODEV);
        }
