...
#if USE_PCI_IO
#define outb(val, addr)     octeon_pci_io_writeb(val, addr)
#define inb(addr)      octeon_pci_io_readb(addr)
#define outw(val, addr)     octeon_pci_io_writew(val, addr)
#define inw(addr)      octeon_pci_io_readw(addr)
#define outl(val, addr)     octeon_pci_io_writel(val, addr)
#define inl(addr)      octeon_pci_io_readl(addr)
#else
#define outb(val, addr)     octeon_pci_mem1_writeb(val, addr)
#define inb(addr)      octeon_pci_mem1_readb(addr)
#define outw(val, addr)     octeon_pci_mem1_writew(val, addr)
#define inw(addr)      octeon_pci_mem1_readw(addr)
#define outl(val, addr)     octeon_pci_mem1_writel(val, addr)
#define inl(addr)      octeon_pci_mem1_readl(addr)
#endif
...
int rtl8139_initialize(bd_t *bis)
{
	...
	while(1){
		...
		dev->recv = rtl_poll;
                ...
                DEBUGF("iobase: 0x%08x, dev->iobase: 0x%08x\n", iobase, dev->iobase);
		eth_register (dev);
...
static int rtl8139_probe(struct eth_device *dev, bd_t *bis)
{
	...
	for (i = 0; i < 3; i++)
		*ap++ = le16_to_cpu (read_eeprom(i + 7, addr_len));
        ...
        DEBUGF("8139 MAC: %02x:%02x:%02x:%02x:%02x:%02x\n",
               dev->enetaddr[0],
               dev->enetaddr[1],
               dev->enetaddr[2],
               dev->enetaddr[3],
...
#ifdef	DEBUG_RX
        uint32_t rx_bus = phys_to_bus((int)rx_ring);
	printf("rx ring address is 0x%08X, bus addr: 0x%08X\n",(unsigned long)rx_ring, rx_bus);
#endif
...
static int rtl_transmit(struct eth_device *dev, volatile void *packet, int length)
{
	...
	DEBUGF("tx buf mem address is 0x%08X, bus addr: 0x%08X\n",(unsigned long)tx_buffer, phys_to_bus((int)tx_buffer));
...
static int rtl_poll(struct eth_device *dev)
{
	...
	ioaddr = dev->iobase;
        ...
        unsigned char chip_cmd = inb(ioaddr + ChipCmd);
	if (chip_cmd & RxBufEmpty) {
		return 0;
	}
...
#ifdef	DEBUG_RX
        printf("rtl_poll: chip_cmd: 0x%x, status: 0x%x\n", chip_cmd, status);
#endif
	...
	rx_status = __le32_to_cpu(*(unsigned int*)(rx_ring + ring_offs));
	rx_size = rx_status >> 16;
	rx_status &= 0xffff;

#ifdef	DEBUG_RX
        printf("(rtl rx_size: %d, at addr 0x%x) ", rx_size, rx_ring + ring_offs);
#endif
	if ((rx_status & (RxBadSymbol|RxRunt|RxTooLong|RxCRCErr|RxBadAlign)) ||
	    (rx_size < ETH_ZLEN) || (rx_size > ETH_FRAME_LEN + 4)) {
		printf("rx error %hX, resetting card\n\n\n", rx_status);
