...
#else 
#define RX_FRAME_PORT (0x00)
#define TX_FRAME_PORT RX_FRAME_PORT
#define TX_CMD_PORT   (0x04)
#define TX_LEN_PORT   (0x06)
#define ISQ_PORT      (0x08)
#define ADD_PORT      (0x0A)
#define DATA_PORT     (0x0C)

static unsigned short readword(int portno)
{
  unsigned short  result;

  result = *(volatile unsigned short *)(CS8900_BASE + portno);
  return (result);
}
static unsigned short readword_swap(int portno)
{
  unsigned short  result;
...
static void writeword(int portno, int value)
{
  *(volatile unsigned char *)(CS8900_BASE + portno) = value>>8;
  *(volatile unsigned char *)(CS8900_BASE + portno) = value;
}
static void writeword_swap(int portno, int value)
{
  *(volatile unsigned char *)(CS8900_BASE + portno) = value;
  *(volatile unsigned char *)(CS8900_BASE + portno) = value>>8;
}
static unsigned short get_reg_init_bus (int regno)
{
  writeword_swap(ADD_PORT, regno);
  return (readword_swap(DATA_PORT));
}
static unsigned short get_reg (int regno)
{
  writeword_swap(ADD_PORT, regno);
...
{
  writeword_swap(ADD_PORT, regno);
  return (writeword_swap(DATA_PORT, val));
}
#endif
...
#else
  status = readword_swap(RX_FRAME_PORT); /* stat */
  rxlen = readword_swap(RX_FRAME_PORT);  /* len */
#endif
...
retry:
	...
	CS8900_TxLEN = length;
#else
  writeword_swap(TX_CMD_PORT, PP_TxCmd_TxStart_Full);
  writeword_swap(TX_LEN_PORT, length);
#endif
	...
	if ((get_reg (PP_BusSTAT) & PP_BusSTAT_TxRDY) == 0) {
		...
		eth_reginit ();
#else
		for (tmo = get_timer (0) + 1 * CFG_HZ; get_timer (0) < tmo;)
#endif
		goto retry;
	}
