...
#else
#define	virt_to_bus(x)	(((unsigned long)x) & 0x3fffffff)
#define phys_to_bus(x)	((x) & 0x3fffffff)
#define bus_to_phys(devno, x) (x)
#endif
...
fill_rx(struct e1000_hw *hw)
{
	...
	rx_tail = (rx_tail + 1) % 8;
	memset(rd, 0, 16);
	rd->buffer_addr = cpu_to_le64(virt_to_bus(packet));
...
e1000_configure_tx(struct e1000_hw *hw)
{
	...
	E1000_WRITE_REG(hw, TDBAL, (u32) virt_to_bus(tx_base));
	E1000_WRITE_REG(hw, TDBAH, 0);

        printf("Set txbase to: 0x%x\n", E1000_READ_REG(hw, TDBAL));
...
e1000_transmit(struct eth_device *nic, volatile void *packet, int length)
{
	...
	while (!(le32_to_cpu(txp->upper.data) & E1000_TXD_STAT_DD)) {
		if (i++ > TOUT_LOOP) {
			DEBUGOUT("e1000: tx timeout (upper.data: 0x%x)\n", le32_to_cpu(txp->upper.data));
