...
-------------------------------------------------------------------------
 * Filename:      jffs2.c
 * Version:       $Id: jffs2_1pass.c 48726 2010-04-26 23:47:32Z rfranz $
 ...
 *
 * $Id: jffs2_1pass.c 48726 2010-04-26 23:47:32Z rfranz $
 *
 */
...
insert_node(struct b_list *list, u32 offset)
{
	...
	if (list->listTail != NULL && list->listCompare(new, list->listTail))
		...
		prev = NULL;
        /* prev is the likely place to start looking in the list for the place it should
        ** go.  prev==NULL means start at head of list */


        /* Go through list (starting at prev or head), looking
        ** for where we should insert the new item */
	...
	if (b != NULL) {
            /* We found two items that we should insert between.  prev goes before new,
            ** and b goes after new.  If b is not found (NULL), then add on to end of list */
		new->next = b;
		if (prev != NULL)
			prev->next = new;
		else
 ...
 * Returns 1 if new should go after old in list (higher version or
 * other compare result.)
 */
static int compare_dirents(struct b_node *new, struct b_node *old)
{
        /* Allocate and read 'extra' bytes from flash as otherwise the
        ** file names are not read, and then the deleted file handling is busted. */
        uint8_t storage_old[sizeof(struct jffs2_raw_dirent) + 100];
        uint8_t storage_new[sizeof(struct jffs2_raw_dirent) + 100];
	struct jffs2_raw_dirent *jNew =
		(struct jffs2_raw_dirent *)get_fl_mem(new->offset, sizeof(struct jffs2_raw_dirent) + 100, storage_new);
	struct jffs2_raw_dirent *jOld =
		(struct jffs2_raw_dirent *)get_fl_mem(old->offset, sizeof(struct jffs2_raw_dirent) + 100, storage_old);
	int cmp;

#ifdef DEBUG_DIRENTS
        printf("compare_dirents: ");
        putnstr(jNew->name, jNew->nsize); puts("  ");
        putnstr(jOld->name, jOld->nsize); puts("\n");
#endif
	 ...
	 * sort by version.  Mark deleted entries in in-memory list by using
     * the datacrc field.  We can't update the inodes since the inodes
     * are in flash.
	 */
	if (jNew->version > jOld->version) {
		 ...
		 */
		jOld->ino = 0;   /* Useless!!!  jOld is on stack */
                old->datacrc = CRC_BAD;

#ifdef DEBUG_DIRENTS
                printf("Marking version %d as invalid, %d is newer\n", jOld->version, jNew->version);
#endif
		return 1;
	}
        new->datacrc = CRC_BAD;
...
jffs2_1pass_find_inode(struct b_lists * pL, const char *name, u32 pino)
{
	...
	for(b = pL->dir.listHead; b; b = b->next, counter++) {
                if (b->datacrc == CRC_BAD)  /* Use CRC_BAD to indicate dir entry unlinked */
                    continue;
...
jffs2_1pass_list_inodes(struct b_lists * pL, u32 pino)
{
	...
	for (b = pL->dir.listHead; b; b = b->next) {
		jDir = (struct jffs2_raw_dirent *) get_node_mem(b->offset,
								pL->readbuf);
		if ((pino == jDir->pino) && (jDir->ino) && b->datacrc != CRC_BAD) { /* ino=0 or CRC_BAD -> unlink */
			u32 i_version = 0;
...
jffs2_1pass_build_lists(struct part_info * part)
{
	struct b_lists *pL;
	struct jffs2_unknown_node *node;
        printf("size: %d, sector_size: %d\n", part->size, part->sector_size);
	...
	pL->readbuf = malloc(max_totlen);
        if (!pL->readbuf)
            printf("ERROR: allocation of pL->readbuf failed\n");
...
#endif /* (CONFIG_COMMANDS & CFG_CMD_JFFS2) */
